...
1. درخواست شما ابتدا به Google Apps Script فرستاده می‌شود
2. از دید فیلتر، این ترافیک شبیه ارتباط عادی با گوگل است
3. Google Apps Script درخواست را به Cloudflare Worker شما ارسال می‌کند
4. Cloudflare Worker سایت مقصد (مثلا اینستاگرام) را دریافت کرده و برمی‌گرداند

## امکانات پروژه

- کاملا رایگان - استفاده از سطوح رایگان گوگل و کلادفلر
- بدون نیاز به سرور یا VPS
- نصب آسان - فقط کافیست کدها را کپی کنید
- امنیت با کلید احراز هویت اختصاصی
- کار با اپ Zyrln روی ویندوز، لینوکس، مک و اندروید
- دسترسی به تمام سایت‌ها (اینستاگرام، توییتر، تلگرام و ...)
...
مشکل: خطای 403 (unauthorized)
راه حل: رمز Auth Key در Zyrln و Apps Script یکی نیست

مشکل: خطای 404 (Not Found)
راه حل: آدرس Worker در کد Apps Script اشتباه وارد شده است

مشکل: هیچ سایتی باز نمی‌شود
راه حل: مطمئن شوید Zyrln در حالت Direct Mode نباشد و پروکسی مرورگر درست تنظیم شده باشد

مشکل: خطای 502 (worker_error)
راه حل: Cloudflare Worker شما کار نمی‌کند. لاگ‌های Worker را در داشبورد کلادفلر بررسی کنید
...
- گوگل و کلادفلر می‌توانند متادیتا (زمان و حجم ترافیک) را ببینند، اما محتوای رمز شده را نمی‌توانند بخوانند
...
**توسعه داده شده با ❤️ برای اینترنت آزاد**
